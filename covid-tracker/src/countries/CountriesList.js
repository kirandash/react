...
const CountriesList = ({ countries = [], onRemovePressed, onPin, onPinClicked }) => { // countries and onRemovePressed are available from mapStateToProps and mapDispatchToProps
    return (
        <div className="countries-list-wrapper">
            {countries.map((country, index) => {
                return <CountryDetail country={country} key={index} onRemovePressed={onRemovePressed} onPin={onPinClicked} />
            })}
        </div>
    );
};
